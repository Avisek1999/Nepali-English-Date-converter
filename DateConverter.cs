...
namespace NepaliDateConverter.Net
{
    ...
    public class DateConverter : IDateConverter
    {
        ...
        private static int[] GetMonthsCountInEnglishYear(bool isLeapYear)
        {
            List<int> list = new List<int>();
            Calendar.conn.Open();
            string sql = "Select * from tblNoOfDaysInEnglishMonths";
            ...
            if(isLeapYear == false)
            {
                foreach (DataRow row in dTable.Rows)
                    list.Add(Convert.ToInt32(row[1]));
            }
            else
            {
                foreach (DataRow row in dTable.Rows)
        ...
        public static DateConverter ConvertToNepali(int yy, int mm, int dd)
        {
            ...
            {
                //english month
                int[] month = GetMonthsCountInEnglishYear(isLeapYear: false);
                //leap year month
                int[] lmonth = GetMonthsCountInEnglishYear(isLeapYear: true);
